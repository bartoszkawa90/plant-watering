...
{
    ESP_LOGI(TAG_wifi, "Got IP event!");
...
esp_err_t example_connect(const char *ssid, const char *passwd)
{
    ...
    ESP_LOGI(TAG_wifi, "Connected to %s", s_connection_name);
    ESP_LOGI(TAG_wifi, "IPv4 address: " IPSTR, IP2STR(&s_ip_addr));
    return ESP_OK;
}
...
esp_err_t example_disconnect(void)
{
    ...
    stop();
    ESP_LOGI(TAG_wifi, "Disconnected from %s", s_connection_name);
