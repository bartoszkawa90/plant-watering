...
{
    ESP_LOGI(TAG_mount, "Initializing SD card");
    ...
    ESP_LOGI(TAG_mount, "Using SDMMC peripheral");
    ...
    ESP_LOGI(TAG_mount, "Using SPI peripheral");
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG_mount, "Failed to initialize bus.");
        return ret;
    }
    ...
    if (ret != ESP_OK){
        if (ret == ESP_FAIL) {
            ESP_LOGE(TAG_mount, "Failed to mount filesystem. "
                "If you want the card to be formatted, set the EXAMPLE_FORMAT_IF_MOUNT_FAILED menuconfig option.");
        } else {
            ESP_LOGE(TAG_mount, "Failed to initialize the card (%s). "
...
esp_err_t example_mount_storage(const char* base_path)
{
    ESP_LOGI(TAG_mount, "Initializing SPIFFS");
    ...
    if (ret != ESP_OK) {
        if (ret == ESP_FAIL) {
            ESP_LOGE(TAG_mount, "Failed to mount or format filesystem");
        } else if (ret == ESP_ERR_NOT_FOUND) {
            ESP_LOGE(TAG_mount, "Failed to find SPIFFS partition");
        } else {
            ESP_LOGE(TAG_mount, "Failed to initialize SPIFFS (%s)", esp_err_to_name(ret));
        }
        return ret;
    }
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG_mount, "Failed to get SPIFFS partition information (%s)", esp_err_to_name(ret));
        return ret;
    }

    ESP_LOGI(TAG_mount, "Partition size: total: %d, used: %d", total, used);
    return ESP_OK;
}
